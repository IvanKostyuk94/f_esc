    ...
    for index, row in df.iterrows():
        ...
        try:
            if len(age) != len(row.per_source):
                raise ValueError(
                    f"The number of age elements and sources should be the same, got {len(age)} and {len(row.per_source)} instead"
                )
        except:
            print(age)
...
def build_star_mass_df(basePath):
    stars_6 = il.snapshot.loadSubset(
        basePath,
        13,
        "stars",
        fields=["ParticleIDs", "Masses", "GFM_StellarFormationTime"],
    )
    stars_8 = il.snapshot.loadSubset(
        basePath,
        8,
        "stars",
        fields=["ParticleIDs", "Masses", "GFM_StellarFormationTime"],
    )
    stars_10 = il.snapshot.loadSubset(
        basePath,
        4,
        "stars",
        fields=["ParticleIDs", "Masses", "GFM_StellarFormationTime"],
    ...
    stars_6_df = pd.DataFrame(
        {"mass": stars_6["Masses"], "formation": stars_6["GFM_StellarFormationTime"]},
        index=stars_6["ParticleIDs"],
    )
    stars_8_df = pd.DataFrame(
        {"mass": stars_8["Masses"], "formation": stars_8["GFM_StellarFormationTime"]},
        index=stars_8["ParticleIDs"],
    )
    stars_10_df = pd.DataFrame(
        {"mass": stars_10["Masses"], "formation": stars_10["GFM_StellarFormationTime"]},
        index=stars_10["ParticleIDs"],
    )
...
def star_mass_gal(ID, redshift, conf, star_df, side_length):
    ...
    path_sources = f"/ptmp/mpa/mglatzle/TNG_f_esc/{conf}/run/L35n2160TNG/{snap}/g{ID}/Input/sources_ic00.in"
    coord = (
        pd.read_csv(path_sources, delim_whitespace=True, header=None, usecols=[0, 1, 2])
        + 0.5
        - side_length / 2
    )
...
def get_simulation_path(halo_id, conf, redshift):
    ...
    simulation_path = os.path.join(
        conf_dir, f"run/L35n2160TNG/{snap}/g{halo_id}/Output/phys_ic00_rt05.out"
    )
    density_path = os.path.join(
        conf_dir, f"run/L35n2160TNG/{snap}/g{halo_id}/Input/dens_ic00.in"
    )
...
def get_average_quantities(halo_id, conf, redshift, df_star_masses):
    ...
    star_mass_in_gal = star_mass_gal(
        halo_id, redshift, conf, star_df=df_star_masses, side_length=dens.shape[0]
    )
...
def update_df(df_name, name_star_mass_dic=None, new_df_name=None, conf="fid2"):
    ...
    for i, halo in df.iterrows():
        ...
        c_gal, sig_gas, sig_star = get_average_quantities(
            halo_id, conf, z, df_star_masses
        )
