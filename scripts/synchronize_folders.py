...
def transfer_to_all_sources(
    origin, destination, data_dir="/ptmp/mpa/mglatzle/TNG_f_esc"
):
...
def correct_fesc_file(
    origin, destination, data_dir="/ptmp/mpa/mglatzle/TNG_f_esc", from_freya=True
):
...
def correct_paths(
    origin,
    destination,
    data_dir="/ptmp/mpa/mglatzle/TNG_f_esc",
    from_freya=True,
    with_dust=False,
):
    ...
    for snap in snap_dirs:
        ...
        for halo in os.listdir(snap_dir_out):
            ...
            if (destination not in os.path.realpath(source_file)) or (not from_freya):
                ...
                print(
                    (destination not in os.path.realpath(source_file))
                    and (not from_freya)
                )
...
correct_paths(origin=origin, destination=destination, from_freya=False, with_dust=True)
