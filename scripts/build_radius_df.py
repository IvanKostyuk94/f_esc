...
def completed_haloIDs(sim_name, snap_name):
    ...
    def test_check(rdir):
        checkfile = _os.path.join(rdir, os.path.basename(exe_path()) + ".check")
    ...
    run_dirs = filter(
        os.path.isdir, _glob.glob(_os.path.join(rundir(sim_name, snap_name), "*"))
    )
    completed = filter(lambda x: test_check(x), run_dirs)
...
def construct_freq_dataframe(dictionary, config, name, simname, fesc_galaxy=False):
    ...
    for i, snapshot in enumerate(dictionary.keys()):
        ...
        for ID in IDs:
            for key in f_esc_radii_elements:
                f_esc_radii_elements[key]["f_esc_elements"].append(
                    input_df.loc[ID, ("f_esc", 4)]["5.0e-2"][f_esc_notation[key]]["cum"]
                )
                f_esc_radii_elements[key]["per_freq_elements"].append(
                    input_df.loc[ID, ("f_esc", 4)]["5.0e-2"][f_esc_notation[key]][
                        "per_freq"
                    ]
                )
                f_esc_radii_elements[key]["per_source_elements"].append(
                    input_df.loc[ID, ("f_esc", 4)]["5.0e-2"][f_esc_notation[key]][
                        "per_source"
                    ]
                )
                f_esc_radii_elements[key]["emitted_photons_elements"].append(
                    input_df.loc[ID, ("f_esc", 4)]["5.0e-2"][f_esc_notation[key]][
                        "emitted_photons"
                    ]
                )
                f_esc_radii_elements[key]["escaped_photons_elements"].append(
                    input_df.loc[ID, ("f_esc", 4)]["5.0e-2"][f_esc_notation[key]][
                        "escaped_photons"
                    ]
                )
                f_esc_radii_elements[key]["frequencies_elements"].append(
                    input_df.loc[ID, ("f_esc", 4)]["5.0e-2"][f_esc_notation[key]][
                        "freqs"
                    ]
                )
                f_esc_radii_elements[key]["n_iterations_elements"].append(
                    input_df.loc[ID, ("f_esc", 4)]["5.0e-2"][f_esc_notation[key]][
                        "n_iterations"
                    ]
                )
        ...
        for key in f_esc_radii:
            f_esc_radii[key]["f_esc"].extend(
                f_esc_radii_elements[key]["f_esc_elements"]
            )
            f_esc_radii[key]["per_freq"].extend(
                f_esc_radii_elements[key]["per_freq_elements"]
            )
            f_esc_radii[key]["per_source"].extend(
                f_esc_radii_elements[key]["per_source_elements"]
            )
            f_esc_radii[key]["emitted_photons"].extend(
                f_esc_radii_elements[key]["emitted_photons_elements"]
            )
            f_esc_radii[key]["escaped_photons"].extend(
                f_esc_radii_elements[key]["escaped_photons_elements"]
            )
            f_esc_radii[key]["frequencies"].extend(
                f_esc_radii_elements[key]["frequencies_elements"]
            )
            f_esc_radii[key]["n_iterations"].extend(
                f_esc_radii_elements[key]["n_iterations_elements"]
            )
    ...
    try:
        dataset_dict = {
            ("ID", 0): all_IDs,
            ("z", 0): redshifts,
            ("HaloMass", 0): halo_masses,
            ("Metallicity", 0): metal,
            ("GasMetallicity", 0): gas_metal,
            ("FractionStars", 0): rel_star_mass,
            ("FractionGas", 0): rel_gas_mass,
            ("FractionDust", 0): rel_dust_mass,
            ("Q0", 0): Q0,
            ...
            ("xHeII", 0): xHeII,
            ("xHeIII", 0): xHeIII,
            ("GridSize", 0): grid_size,
            ("BHMass", 0): bh_mass,
            ("BHGrowth", 0): bh_growth,
            ("SFR", 0): sfr,
            ("density", 0): density,
            ("clumping", 0): clumping,
        }
...
def build_df(
    run_names=["fid2"], filename=None, simname="L35n2160TNG", fesc_galaxy=False
):
